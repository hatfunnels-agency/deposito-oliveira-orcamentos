...
;
  ...
  const imprimirOrcamento = (detalhe?: OrcamentoDetalhe | null) => {
    ...
    const itensHtml = d
      ? d.orcamento_itens.map(i => `<tr><td style="padding:2px 4px;border-bottom:1px solid #eee">${i.produto_nome}</td><td style="padding:2px 4px;border-bottom:1px solid #eee;text-align:center">${i.quantidade}</td><td style="padding:2px 4px;border-bottom:1px solid #eee;text-align:center">${i.unidade}</td><td style="padding:2px 4px;border-bottom:1px solid #eee;text-align:right">R$ ${formatBRL(i.preco_unitario)}</td><td style="padding:2px 4px;border-bottom:1px solid #eee;text-align:right">R$ ${formatBRL(i.subtotal)}</td></tr>`).join('')
      : itens.map(i => `<tr><td style="padding:2px 4px;border-bottom:1px solid #eee">${i.produto.nome}</td><td style="padding:2px 4px;border-bottom:1px solid #eee;text-align:center">${i.quantidade}</td><td style="padding:2px 4px;border-bottom:1px solid #eee;text-align:center">${i.produto.unidade}</td><td style="padding:2px 4px;border-bottom:1px solid #eee;text-align:right">R$ ${formatBRL(i.produto.preco)}</td><td style="padding:2px 4px;border-bottom:1px solid #eee;text-align:right">R$ ${formatBRL(i.produto.preco * i.quantidade)}</td></tr>`).join('');
    ...
    const htmlImp = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Orçamento ${cod}</title><style>@page{size:A5;margin:10mm}*{box-sizing:border-box}body{font-family:Arial,sans-serif;font-size:10px;color:#333;margin:0;padding:0}.hdr{display:flex;align-items:center;gap:8px;margin-bottom:5px}.hdr img{height:38px;width:auto}.hdr h1{margin:0;font-size:13px;color:#F7941D}.hdr p{margin:1px 0;color:#666;font-size:8.5px}hr{border:none;border-top:1.5px solid #F7941D;margin:5px 0}.ig{display:grid;grid-template-columns:1fr 1fr;gap:0 12px;margin:3px 0}.ir{font-size:9px;line-height:1.5}.full{grid-column:1/-1}table{width:100%;border-collapse:collapse;margin:5px 0;font-size:8.5px}th{background:#F7941D;color:white;padding:3px 4px;text-align:left}td{padding:2px 4px;border-bottom:1px solid #eee}.tr{text-align:right}.tc{text-align:center}tfoot td{font-weight:bold;border-top:2px solid #F7941D;border-bottom:none}.totrow td{font-size:11px;color:#F7941D;padding:3px 4px}.pagto{margin:4px 0;padding:4px 6px;border:1px solid #ddd;border-radius:3px;background:#fffbf0;font-size:8.5px}.parc{color:#666;font-size:7.5px;margin-top:2px}.ftr{margin-top:4px;padding-top:4px;border-top:1px solid #ddd;font-size:7.5px;color:#999;text-align:center}</style></head><body><div class="hdr"><img src="${logoBase64||'/logo.png'}" alt="Logo"/><div><h1>Depósito Oliveira</h1><p>Materiais de Construção</p><p>Av. Inocêncio Seráfico, 4020 - Centro | Carapicuíba - SP, 06380-021</p><p>Tel: (11) 4187-1801</p></div></div><hr/><div class="ig">${cod?'<div class="ir"><b>Código:</b> '+cod+'</div>':''}<div class="ir"><b>Data:</b> ${dataCriacao}</div><div class="ir"><b>Cliente:</b> ${nome}</div>${tel?'<div class="ir"><b>Telefone:</b> '+tel+'</div>':''}<div class="ir"><b>Entrega:</b> ${tipo==='entrega'?'Entrega no endereço':'Retirada na loja'}</div>${tipo==='entrega'&&end?'<div class="ir full"><b>Endereço:</b> '+end+'</div>':''}${dataEnt?'<div class="ir"><b>Data entrega:</b> '+new Date(dataEnt+'T12:00:00').toLocaleDateString('pt-BR')+'</div>':''}${dataRet?'<div class="ir"><b>Data retirada:</b> '+new Date(dataRet+'T12:00:00').toLocaleDateString('pt-BR')+'</div>':''}${obsImp?'<div class="ir full"><b>Obs:</b> '+obsImp+'</div>':''}${formaPagImp?'<div class="ir"><b>Pagamento:</b> '+(formaPagLabelImp[formaPagImp]||formaPagImp)+'</div>':''}</div><table><thead><tr><th>Produto</th><th class="tc">Qtd</th><th class="tc">Un</th><th class="tr">Unit.</th><th class="tr">Total</th></tr></thead><tbody>${itensHtml}</tbody><tfoot><tr><td colspan="4" class="tr">Subtotal:</td><td class="tr">R$ ${formatBRL(sub)}</td></tr>${tipo==='entrega'&&frete>0?'<tr><td colspan="4" class="tr">Frete:</td><td class="tr">R$ '+formatBRL(frete)+'</td></tr>':''}<tr class="totrow"><td colspan="4" class="tr">TOTAL:</td><td class="tr">R$ ${formatBRL(tot)}</td></tr></tfoot></table><div class="pagto"><strong>&#128181; À vista: R$ ${formatBRL(tot)}</strong> &nbsp;|&nbsp; <strong>&#128179; Cartão (+8%): R$ ${formatBRL(valorCartaoImp)}</strong><div class="parc">${Array.from({length:MAX_PARCELAS},(_,i)=>i+1).map(n=>n+'x R$ '+formatBRL(valorCartaoImp/n)).join(' | ')}</div></div><div class="ftr">Orçamento válido por 7 dias &middot; Sujeito à disponibilidade de estoque</div></body></html>`;
